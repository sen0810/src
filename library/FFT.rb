  	...
  	while n < leng do
  		...
  		if(values.size() == @windowWidth) then
  			if(@windowName == "hm") then
  				...
  				result[result.size()] = fft(BlackmanWindow(values,n%leng))
  			elsif(@windowName == "bh") then
  				result[result.size()] = fft(BartlettHannWindow(values,n%leng))
  			else
  				result[result.size()] = fft(rectangularWindow(values,n%leng))
  			end
  			temp = []
